...
use msgpack;
use msgpack::{Error, ReadError, MarkerError};
...
fn from_bin8_min_read_len() {
    ...
    assert_eq!(0, msgpack::read_bin_len(&mut cur).unwrap());
...
fn from_bin8_max_read_len() {
    ...
    assert_eq!(255, msgpack::read_bin_len(&mut cur).unwrap());
    assert_eq!(2, cur.position());
}
...
fn from_bin8_eof_read_len() {
    ...
    assert_eq!(Error::InvalidDataRead(ReadError::UnexpectedEOF),
        msgpack::read_bin_len(&mut cur).err().unwrap());
...
fn from_bin16_max_read_len() {
    ...
    assert_eq!(65535, msgpack::read_bin_len(&mut cur).unwrap());
...
fn from_bin32_max_read_len() {
    ...
    assert_eq!(4294967295, msgpack::read_bin_len(&mut cur).unwrap());
